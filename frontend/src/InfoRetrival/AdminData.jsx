...
const AdminData = () => {
  ...
  console.log("Retrieving Admin Data");
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("https://employee-management-eyf8.onrender.com/api/get-details/admins");
  ...
  useEffect(() => {
    let result = employees.filter((emp) =>
      ...
      emp.Admin_ID.toLowerCase().includes(searchTerm.toLowerCase())
    );
  ...
  return (
    <div className="p-6 bg-gradient-to-r from-blue-50 to-indigo-100 min-h-screen">
      <h1 className="text-4xl font-bold text-center text-blue-800 mb-8">Admins</h1>
      ...
      {/* Status */}
      {loading ? (
        <div className="text-center text-lg text-blue-600">Loading Admins...</div>
      ) : filteredEmployees.length === 0 ? (
        <div className="text-center text-red-600 font-medium">No matching Admin found.</div>
      ) : (
        <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
          {filteredEmployees.map((emp) => (
            ...
            >
              ...
              <p className="text-gray-600"><span className="font-semibold">ID:</span> {emp.Admin_ID}</p>
